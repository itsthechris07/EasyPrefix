    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                sender.sendMessage("Groups: " + GroupHandler.getGroups().size() + "/" + GroupHandler.getSubgroups().size());
                sender.sendMessage("Users cached: " + User.getUsers().size());
                sender.sendMessage("Genders cached: " + Gender.getTypes().size());
                sender.sendMessage("Bukkit Version: " + Bukkit.getVersion());
                sender.sendMessage("Version Name: " + Bukkit.getBukkitVersion());
                sender.sendMessage("active EventHandler: " + HandlerList.getRegisteredListeners(EasyPrefix.getInstance().getPlugin()).size());
                return true;
                ...
                sender.sendMessage(" ");
                sender.sendMessage("§7/§5EasyPrefix database migrate §f| §7save groups and users to database (will override database!)");
                sender.sendMessage(" ");
            ...
            if (args[0].equalsIgnoreCase("set")) {
                if (sender.hasPermission("EasyPrefix.admin")) {
                    ...
                    if (targetPlayer != null) {
                        User target = User.getUser(targetPlayer);
