...
public class ConfigData {
    ...
    public enum ConfigKeys {
        ENABLED("enabled"), LANG("lang"), CUSTOM_PREFIX("user.custom-prefix.enabled"), HANDLE_CHAT("chat.handle-chat"), COLOR_RAINBOW_COLORS("chat.color.rainbow.colors"), GUI_SHOW_ALL_CHATCOLORS("gui.show-all-chatcolors"), USE_GENDER("gender.enabled"), FORCE_GENDER("gender.force-gender"), USE_SUBGROUPS("subgroups.enabled"), USE_JOIN_QUIT("join-quit-messages.enabled"), HIDE_JOIN_QUIT("join-quit-messages.hide-messages"), JOIN_QUIT_SOUND_RECEIVER("join-quit-messages.sound.receiver"), USE_JOIN_SOUND("join-quit-messages.sound.join.enabled"), JOIN_SOUND("join-quit-messages.sound.join.sound"), USE_QUIT_SOUND("join-quit-messages.sound.quit.enabled"), QUIT_SOUND("join-quit-messages.sound.quit.sound"), USE_SQL("sql.enabled"), SQL_HOST("sql.host"), SQL_PORT("sql.port"), SQL_DATABASE("sql.database"), SQL_USERNAME("sql.username"), SQL_PASSWORD("sql.password"), SQL_TABLE_PREFIX("sql.table-prefix"), HANDLE_COLORS("chat.handle-colors");
